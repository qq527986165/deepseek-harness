...
| `ctx.memory` | `seam` | [`memory`](../packages/memory/memory) | [`memory-local`](../packages/memory/memory-local) | [`tool-memory`](../packages/memory/tool-memory), [`memory-lifecycle`](../packages/memory/memory-lifecycle), [`memory-remote`](../packages/memory/memory-remote) | - | The sole-provider seam resolves session vault scopes; the file-first provider owns the Obsidian-compatible vaults and their derived SQLite index. |
