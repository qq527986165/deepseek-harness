...
| `@deepseek-ai/dsh-tool-memory` | `memory_delete`、`memory_read`、`memory_search`、`memory_traverse`、`memory_write` | `ctx.tools`、`ctx.memory`、`a calling Agent (exec.agent supplies the session cwd)` | `tool/call`、`tool/result` | - | - |
...
按 id 或精确标题删除一篇记忆笔记（软删除：文件会移到同级的回收文件夹）。始终要求用户确认。
...
```json
{
  ...
  "properties": {
    "ref": {
      "type": "string",
      "description": "Note id or exact title."
    },
    "scope": {
      "type": "string",
      "description": "Optional vault scope: \"project\" or \"global\". Omit to resolve across both, project first."
    }
  },
  "required": [
    "ref"
...
在项目与全局 vault 中搜索记忆（项目命中排前），返回带片段的排序命中。
...
```json
{
  ...
  "properties": {
    "query": {
      "type": "string",
      "description": "Search terms; results must contain them."
    },
    "limit": {
      "type": "integer",
      "description": "Optional maximum hit count; bounded by deployment config."
    }
  },
  "required": [
    "query"
...
列出链接到一篇记忆笔记的笔记——外向的 wikilink/related 目标与内向回链——向外走一到两跳。
...
```json
{
  ...
  "properties": {
    "ref": {
      "type": "string",
      "description": "Start note id or exact title."
    },
    "depth": {
      "type": "integer",
      "description": "Hops to walk: 1 (default) or 2."
    },
    "kinds": {
      "type": "array",
      "description": "Optional link kinds to include: \"wikilink\" and/or \"related\". Default: both.",
      "items": {
        "type": "string"
      }
    }
...
创建记忆笔记或按 id 替换。笔记保存关于用户、偏好与项目决策的持久事实。用户要求记住某事时调用；每篇笔记保持一个主题，并用 [[wikilinks]] 链接相关笔记。
...
```json
{
  ...
  "properties": {
    "id": {
      "type": "string",
      "description": "Optional id of an existing note to replace. Omit to create a new note."
    },
    "scope": {
      "type": "string",
      "description": "Optional vault scope: \"project\" (default when the session has a project) or \"global\"."
    },
    "title": {
      "type": "string",
      "description": "Exact note title; wikilinks and lookups resolve by this title."
    },
    "content": {
      "type": "string",
      "description": "Note body in markdown; may contain [[wikilinks]]."
    },
    "tags": {
      "type": "array",
    ...
    "related": {
      "type": "array",
      "description": "Optional titles of related notes to link.",
      "items": {
        "type": "string"
      }
    }
