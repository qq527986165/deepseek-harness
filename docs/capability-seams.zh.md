...
| `ctx.memory` | `seam` | [`memory`](../packages/memory/memory) | [`memory-local`](../packages/memory/memory-local) | [`tool-memory`](../packages/memory/tool-memory), [`memory-lifecycle`](../packages/memory/memory-lifecycle) | - | 唯一提供方 seam 解析会话 vault 作用域；文件优先提供方拥有 Obsidian 兼容的 vault 及其派生的 SQLite 索引。 |
