...
flowchart LR
  ...
  pkg_memory["memory"]
  svc_memory["ctx.memory<br/>Memory capability seam"]
  ...
  svc_memory --> pkg_memory_lifecycle
...
| `ctx.memory` | `seam` | [`memory`](../packages/memory/memory) | [`memory-local`](../packages/memory/memory-local) | [`tool-memory`](../packages/memory/tool-memory), [`memory-lifecycle`](../packages/memory/memory-lifecycle), [`memory-remote`](../packages/memory/memory-remote) | - | 唯一提供方 seam 解析会话 vault 作用域；文件优先提供方拥有 Obsidian 兼容的 vault 及其派生的 SQLite 索引。 |
