...
记忆能力 seam：唯一提供方注册、会话作用域解析，以及 write/read/search/traverse 服务接口。服务拥有产品规则——会话可以触达哪些 vault——而注册的提供方拥有每个 vault 内的文件布局与索引。存储是文件优先的 markdown，可在 Obsidian 中编辑、可在 git 中做 diff。
...
- **暂无会话事件** —— Phase 2 增加 `memory/*` 会话事件，使每次蒸馏写入与注入的上下文可从日志重建；Phase 1 仅依赖工具调用/结果日志。
