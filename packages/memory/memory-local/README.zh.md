...
- **蒸馏提交是整轮事务。** `commitDistill` 同时持有所有参与 vault 的独占链，暂存每个带短 ID 后缀的新节点和每个作用域一条带锚点的日记条目，发布并建立索引，再验证精确读取与双向链接，最后标记事务已提交。任何提交前失败都会在所有作用域恢复原日记字节并移除全部新节点；持久 manifest 在下次打开 vault 时恢复中断的发布。同标题旧节点保持不变，并成为新节点按精确路径链接的前序。
- **生命周期读取。** `readPersona` 完整读取 `MEMORY.md`；`recentNotes` 列出 `notes/` 下的主题笔记，最新在前，受配置上限约束。
- **列表。** `listNotes` 返回 vault 的可列表行——`MEMORY.md` 置顶，随后是 `notes/` 行按 `updated` 倒序，日记排除——每行带首行摘录；被收养文件以 `adopted:` 身份加入。显式 `limit` 在 `maxListNotes` 之下被遵守，非法值显式失败。
- **软删除。** `delete` 把文件移入 vault 外的同级 `memory-trash/` 文件夹（文件名带时间戳、碰撞加后缀），清除笔记的索引行与每条入向链接行，存留的 wikilink 在重建时悬空。`mode: 'permanent'` 为升级流程彻底移除文件——其内容已存在于全局 vault。
...
- **恢复是手动的** —— 被回收的文件只能通过手动移动文件（或 Obsidian）移回；尚无恢复原语或回收保留策略。
