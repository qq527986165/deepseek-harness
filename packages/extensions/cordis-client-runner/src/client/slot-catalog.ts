...
export const CLIENT_SLOT_API: readonly ClientSlotEntry[] = [
  ...
  },
  {
    ...
    keyDomain: 'fixed by the owner\'s key table { [Kind in ChatNodeKind]: { node: ChatNode<Kind> } }, already taken: assistant-step, command, command-input, compaction, context, manual-compaction, memory-distill, memory-review, model-retry, steering, tool-call, turn-error, turn-max-tokens, turn-tail, unknown, user, workflow-run',
    hookContext: 'string',
    ...
    occupants: [
      ...
      'client-ui-memory MemoryReviewNode key \'memory-review\'',
      'client-ui-memory MemoryDistillNode key \'memory-distill\'',
  ...
  },
  {
    ...
    keyDomain: 'open: any string the owner dispatches (no compile-time key set), already taken: memory-lifecycle',
    hookContext: '',
    slotInject: '',
    declaredBy: 'an entry in \'settings.plugins.tab\' (client-ui-settings-plugins), so it exists while that entry is mounted',
    occupants: [
      'client-ui-memory MemorySettingsCard key \'memory-lifecycle\'',
  ...
  },
  {
    ...
    declaredBy: 'an entry in \'root\' (client-ui-layout), so it exists while that entry is mounted',
    occupants: [
      'client-ui-memory MemoryPanel id \'memory\'',
    ],
    replaceRisk: 'none',
  ...
  },
  {
    ...
    declaredBy: 'an entry in \'sidebar\' (client-ui-sidebar), so it exists while that entry is mounted',
    occupants: [
      'client-ui-memory MemoryFooterAction id \'memory\'',
