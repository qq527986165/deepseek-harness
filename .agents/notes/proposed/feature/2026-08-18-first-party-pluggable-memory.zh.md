...
- **自动加载。** 会话开始时，提供方把全局与项目人设笔记全文加一个笔记目录（标题、tags、更新日期、首行摘录）注入首次请求。
- **自动写入。** 每轮从用户提问到助手回答结束后，一次辅助蒸馏调用要么把一篇或多篇新节点记忆连同对应日记条目一起提交，要么什么都不写；同一调用内的分类器把个人事实与偏好路由到全局 vault，把项目事实留在项目 vault。
...
记忆保持两个轴。`notes/` 下的节点记忆承载蒸馏出的事实——身份、偏好、决策——作为便于查找的小型召回单元，正文详略由 `concise`/`detailed` 模式控制。日记记忆承载时间叙事：按用户或主机配置时区的自然日每天一个文件，每个需要记忆的回合追加一个条目，链接到本回合创建的全部节点。每个节点再通过稳定的条目锚点反向链接到对应日记条目，因此一天的日记文件对应多个节点。后续回合不合并或改写旧节点：同类内容仍创建新节点和新日记条目；能可靠判断为延续时，新节点可以链接前序节点，旧文件保持不动，由索引 backlinks 提供正向追踪。节点文件名由简短概括加短稳定 id 组成，避免同名覆盖。提供方递归索引 vault 根下的 markdown 文件，因此 `notes/` 与 `journal/` 子目录是约定而非格式规则，把 `dir` 指向一个已存在的 markdown 文件夹即收养其中文件为记忆。提供方在自身节点写入时负责 `id`/`created`/`updated`；外部编辑按文件状态更新索引，提供方不重写该文件。wikilink 允许悬空，遍历时标记为未解析。插件绝不触碰 git 配置：提交项目 vault 是文档化的团队记忆共享方式，忽略它则保持记忆私有。
...
- **会话开始。** `agent/session-start` 上的监听器完整读取全局与项目的 `MEMORY.md`，外加每个已解析作用域节点记忆的目录（标题、tags、更新日期、首行摘录，最新在前，作为一条消息字节封顶），交给 `agent.inject()`，于是首次请求把它们看作持久的注入上下文——作为普通注入的用户内容落日志，满足模型可见即落日志。已加载笔记被 watcher 观察到变更时以同样方式注入（AGENTS.md 子目录变更模式）。日记不进入注入集合；召回通过搜索到达它，模型经 `memory_read` 按需取整篇笔记内容。
- **每回合蒸馏。** `turn/end` 上的监听器对刚完成的“用户提问至助手回答结束”回合发起一次非阻塞的辅助 LLM 调用（压缩 summarizer 的架构：重放前缀加缓存友好的尾部指令）。调用产出零篇或多篇候选节点记忆——事实、决策、偏好——并把每篇分类为 `project` 或 `global`：个人身份、偏好与跨项目规则归全局；项目事实留项目。零候选会丢弃整次蒸馏：不写节点、不追加日记、不产生 `memory/distill` 事件。否则 lifecycle 按 scope 分组，把各组新节点与该 scope 当日日记中的一个条目共同提交；项目组和全局组绝不跨 vault 互链。id、路径、条目锚点、节点↔日记双向链接与可选的新节点→前序节点链接都由 host 在真实产物确定后生成，模型不直接声明最终链接。一轮对话跨所有参与 scope 形成一次原子提交：任何文件写入、索引更新或回读/链接校验失败都不产生可见成功，并由恢复流程回滚或补完。只有全部真实文件非空、已索引、可读取且互链可解析后，lifecycle 才追加 `memory/distill`；该事件是携带稳定节点引用和精确日记条目引用的提交凭证，不是进度或部分失败记录。
- **成本与噪声控制。** 蒸馏可配置开关（开/关、参与蒸馏的最小回合长度、每回合 token 上限，以及一个 `concise`/`detailed` 指令模式切换）；辅助调用默认每回合一次。
...
- **纯日记式笔记（WorkBuddy 文件风格）。** 作为唯一轴不予采纳：只有日记会失去事实的精确召回与图谱中心性。混合形态保留日记的时间顺序与叙事，而事实留在互链的节点记忆里；其边际成本是设计本就支付的蒸馏调用里多一个输出字段。
...
10. Phase 2 验收（现在记录、届时验证）：回放驱动的组装测试展示两份人设笔记的会话开始注入；无持久记忆的回合不写节点、日记条目或事件；需要记忆的回合先原子创建 scope 内节点与双向日记条目链接，再出现一条 `memory/distill` 提交凭证。项目/全局混合回合要么两个 vault 分组都提交，要么都不提交；事件能重建全部稳定引用；后续同类记忆创建带前序链接的新节点而不合并旧节点；日记不进入注入集合。

## Risks

- **Vault/索引漂移。** 外部编辑器与崩溃可能让索引失同步；派生状态设计把损失限制在重建以内，mtime 复查让过期状态只是暂时的。
- **并发编辑与多文件提交。** Obsidian 与 git 同步可能在蒸馏暂存节点和日记条目时写入。提供方跨所有参与 vault 串行化整轮提交，在发布事件前校验暂存文件与索引，并以恢复元数据补完或回滚中断提交；外部编辑仍经索引对账，不能被静默覆盖。
- **蒸馏成本与质量。** 每回合一次辅助 LLM 调用是真实的每回合成本；配置开关（关闭项、最小回合长度、token 上限）约束它。追加式节点保留历史但可能重复相关事实，因此用前序链接和后续节点视角承载连续性，而不原地合并。
