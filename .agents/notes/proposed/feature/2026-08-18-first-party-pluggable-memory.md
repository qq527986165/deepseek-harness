...
- **Auto-load.** When a session starts, the provider injects the global and project persona notes plus a recency window of project notes into the first request.
- **Auto-write.** After every turn, an auxiliary distillation call summarizes the turn into topic notes and writes them silently; a classifier inside the same call routes personal facts and preferences to the global vault and project facts to the project vault.
...
Memory keeps two axes. Topic notes hold distilled facts — identity, preferences, decisions — each a stable unit of recall. The journal holds the chronological narrative: one file per active day, each turn appending a short task summary that links to the topic notes it touched. Facts live only in topic notes, so journal entries point instead of restate; the journal is the human-readable changelog and the temporal spine of the link graph. The provider indexes markdown files recursively under the vault root, so the `notes/` and `journal/` subfolders are convention, not format rules, and pointing `dir` at an existing markdown folder adopts its files as memory. Topic-note filenames derive from the title at creation and are stable; renaming with link repair is deferred. The provider owns `id`/`created`/`updated` on its own topic-note writes; external edits update the index without the provider rewriting the file. Wikilinks may dangle and traversal marks them unresolved. The plugin never touches git configuration: committing the project vault is the documented way to share team memory, ignoring it keeps memory private.
...
- **Session start.** A listener on `agent/session-start` reads the global and project `MEMORY.md` plus a configurable recency window of project topic notes (title and body, byte-capped) and hands them to `agent.inject()`, so the first request sees them as durable injected context — logged as ordinary injected user content, satisfying model-visible-means-logged. A watcher change to a loaded note injects the same way (the AGENTS.md subdirectory-change pattern). The journal stays out of the injected set; recall reaches it through search.
- **Every-turn distillation.** A listener on `turn/end` starts one non-blocking auxiliary LLM call over the finished turn (the compaction summarizer's architecture: replayed prefix, cache-friendly trailing instruction). The call emits candidate topic notes — facts, decisions, preferences — and classifies each as `project` or `global` in the same pass: personal identity, preferences, and cross-project rules go global; project facts stay project. The same call also emits one journal entry — a short task narrative linking the topic notes it touched — appended to that day's journal file through the provider's exclusive write chain, so concurrent sessions serialize appends instead of corrupting the file. Candidates then write silently through the service with the merge-don't-restate rule on existing notes. A `memory/distill` session event records what was written so the log reconstructs every memory mutation.
- **Cost and noise controls.** Distillation is config-gated (on/off, minimum turn length to distill, per-turn token cap); the auxiliary call is one per turn by default.
...
10. Phase 2 acceptance (recorded now, verified then): a replay-driven assembled test shows session-start injection of both persona notes, a per-turn distillation writing a project note and appending a journal entry that links it, a classifier routing a personal fact to the global vault, a `memory/distill` event reconstructing the write, and the journal excluded from the injected set.
...
- **Concurrent edits.** Obsidian and git sync may write while the provider writes; last-write-wins at file level with index reconciliation, and the provider never rewrites a file it did not last write; journal appends serialize through the provider's single exclusive write chain.
- **Distillation cost and quality.** One auxiliary LLM call per turn is a real per-turn cost; config gates (off switch, minimum turn length, token cap) bound it, and merge-don't-restate keeps notes from accumulating restatements.
